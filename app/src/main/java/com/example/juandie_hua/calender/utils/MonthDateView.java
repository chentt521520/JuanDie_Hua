...
public class MonthDateView extends View {
	...
	private int mSelectBGColor = Color.parseColor("#add4cf");
	...
	public MonthDateView(Context context, AttributeSet attrs) {
		...
		year = c.get(Calendar.YEAR);// 获取当日期
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int day = 0; day < mMonthDays; day++) {
			...
			if (dayString.equals(mSelDay + "")) {
				...
				if ((mSelMonth + 1) == mMonth && mCurrYear == mSelYear) {
					if (Integer.valueOf(dayString).intValue() < mDay) {
	...
	public void onLeftClick() {
		...
		if ((year == mYear) && (month == mMonth - 1)) {
			...
			if (month == 0) {// 若果是1月份，则变成12月份
				...
				if (day <= mDay) {
					month = month - 1;
					day = DateUtils.getMonthDays(mDay, month);
					day = mDay;
				} else {
					month = month - 1;
//					day = DateUtils.getMonthDays(year, month);
