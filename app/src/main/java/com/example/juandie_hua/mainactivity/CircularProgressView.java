...
public class CircularProgressView extends View {
	...
	public CircularProgressView(Context context, AttributeSet attrs,
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.CircularProgressView, defStyleAttr, 0);

		mBorderWidth = a.getDimension(
				R.styleable.CircularProgressView_borderWidth2, getResources()
						.getDimension(R.dimen.circular_default_border_width));

		angleAnimatorDuration = a
				.getInt(R.styleable.CircularProgressView_angleAnimationDurationMillis,
						getResources()
								.getInteger(
										R.integer.circular_default_angleAnimationDurationMillis));

		sweepAnimatorDuration = a.getInt(
				R.styleable.CircularProgressView_sweepAnimationDurationMillis,
				getResources().getInteger(
						R.integer.circular_default_sweepAnimationDuration));

		minSweepAngle = a.getInt(
				R.styleable.CircularProgressView_minSweepAngle, getResources()
						.getInteger(R.integer.circular_default_miniSweepAngle));

		int colorArrayId = a.getResourceId(
				R.styleable.CircularProgressView_colorSequence,
				R.array.circular_default_color_sequence);
		if (isInEditMode()) {
			mColors = new int[1];
			mColors[0] = getResources().getColor(Color.parseColor("#b1b0b0"));
		} else {
	...
	public void draw(Canvas canvas) {
		...
		if (mModeAppearing) {
			mPaint.setColor(Color.parseColor("#b1b0b0"));// 设置圈圈颜色
	...
	private Property<CircularProgressView, Float> mAngleProperty = new Property<CircularProgressView, Float>(
			Float.class, "angle") {
		@Override
	...
	private Property<CircularProgressView, Float> mSweepProperty = new Property<CircularProgressView, Float>(
			Float.class, "arc") {
		@Override
		public Float get(CircularProgressView object) {
