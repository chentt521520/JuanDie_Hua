...
public class myOrderAll extends Fragment implements re_jk {
	View v;
	myorder_adapter ada;
	List<myorder_adaData> list;

	@ViewInject(R.id.morderall_listv)
	ListView listv_v;

	@ViewInject(R.id.morderall_nogoods)
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (v == null) {
			v = inflater.inflate(R.layout.myorderall, container, false);
			x.view().inject(this, v);
			preferences = PreferenceManager
					.getDefaultSharedPreferences(getActivity());
			handler = new MyHandler(myOrderAll.this);
	...
	private void xutils_getorderall(final int page) {
		...
		final String url = "https://app.juandie.com/api_mobile/user.php?act=order_list&order_status=all&page="+page;
	...
	public static class MyHandler extends Handler {
		WeakReference<myOrderAll> referenceObj;

		public MyHandler(myOrderAll activity) {
			referenceObj = new WeakReference<myOrderAll>(activity);
		}

		@Override
		public void handleMessage(Message msg) {
			final myOrderAll activity = referenceObj.get();
			switch (msg.what) {
			case 0x01:
