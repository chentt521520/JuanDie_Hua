...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
	...
	private int WHEEL = 1000; // 转动
	private int WHEEL_WAIT = 1001; // 等待
	...
	public void setData(List<ImageView> views, List<ADInfo> list, ImageCycleViewListener listener, int showPosition) {
		...
		for (int i = 0; i < indicators.length; i++) {
			...
			view.setPadding(0, 0, (int) (wide * 10 / 750.0), (int) (wide * 5 / 750.0));

			LayoutParams lp = new LayoutParams((int) (wide * 20 / 750.0),
					(int) (wide * 15 / 750.0));
	...
	public void releaseHeight() {
		getView().getLayoutParams().height = LayoutParams.MATCH_PARENT;
		refreshData();
	}
